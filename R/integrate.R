...
.onAttach <- function (...) {
  packageStartupMessage("nhlscrapr v 1.10.5")
}
...
full.game.database <- function (extra.seasons=0) {
  ...
  date2 <- nhlscrapr::date201415[order(nhlscrapr::date201415$gcode),]
  gamecols <- match(paste0("20142015",as.character(date2$gcode)), paste0(game.roster$season, game.roster$gcode))
  game.roster$awayteam[gamecols] <- as.character(date2$awayteam)
  game.roster$hometeam[gamecols] <- as.character(date2$hometeam)
  unplayed <- which(game.roster$game.start[gamecols] == "")
  game.roster$game.start[gamecols[unplayed]] <- paste(as.character(date2$StartET[unplayed]),"ET")
  game.roster$date[gamecols[unplayed]] <- as.character(date2$GameDate[unplayed])
...
process.single.game <- function (season="20122013", gcode="20001",
                                 ...
                                 save.to.file=TRUE, ...) {
  #season="20142015"; gcode="30321"; rdata.folder="../../nhlr-data"; override.download=FALSE; save.to.file=TRUE
...
construct.rosters.from.list <- function (roster.collection,  #raw list
  ...
  ## roster.collection =NULL; roster.master=NULL; roster.dropin=get(load("../../source-data/roster-dropin.RData"))
    ...
    if (!is.null(roster.dropin)) {
        ...
        match1 <- match(roster.dropin$numfirstlast,
                        roster.master$numfirstlast)
        
        if (any(is.na(match1))) {
            newrecs <- roster.dropin[is.na(match1),]
